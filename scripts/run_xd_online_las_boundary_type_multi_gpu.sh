...
CENTER_LINE_DIR="/nas/nfs/large-model/vince/data/xd-online-las-data/test-v2/center_line_2d"
OUTPUT_DIR="/nas/nfs/large-model/vince/data/xd-online-las-data/test-v2/inference"
PROMPT_FILE="${PROJECT_ROOT}/prompt-perception/prompt-classification-boundary-type.txt"
USER_GPU_IDS=""
USER_NUM_GPUS=""
REMAINING_ARGS=()
...
count_center_line_files() {
  find "${CENTER_LINE_DIR}" -maxdepth 1 -type f -name '*.json' | wc -l | tr -d ' '
}
...
total_items="$(count_center_line_files)"
if [[ "${total_items}" -le 0 ]]; then
  echo "[multi-gpu] no center-line JSON files found: ${CENTER_LINE_DIR}" >&2
  exit 1
fi
...
log_dir="${MULTI_GPU_LOG_DIR:-${OUTPUT_DIR}/_multi_gpu_logs/boundary_${timestamp}}"
mkdir -p "${log_dir}"

echo "[multi-gpu] center_line_files=${total_items} workers=${worker_count} gpus=${gpu_ids[*]:-none}"
...
for worker_idx in $(seq 0 $((worker_count - 1))); do
  ...
  command=(
    ...
    --image-dir "${IMAGE_DIR}"
    --center-line-dir "${CENTER_LINE_DIR}"
